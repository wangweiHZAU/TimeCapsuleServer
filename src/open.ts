...
{
  let tt = document.querySelector("input[name='passwd'") as HTMLInputElement;
  let passwd = tt.value;
  ...
  if (cap)
  {
    ...
    if (check_time(cap.time))
    {
      let say = document.querySelector(".cap-tips");
      say?.classList.remove("hide");
      let words = document.querySelector(".cap-content") as HTMLInputElement;
      words?.classList.remove("hide");
      if (say) say.innerHTML = `${cap.name} 在 ${cap.time} 对你说：`;
  ...
  {
    let t = document.querySelector("input[name='passwd']") as HTMLInputElement;
    t.value = "";
